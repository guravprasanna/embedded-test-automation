...
def run_test(devices):
    ...
    for device in devices:
        if device["device_id"] == device_id:
            ...
            test_report["failed_test"] = (device["total_test"] - device["passed_test"])
            test_report["power"] = device["voltage"] * device["current"]
            test_report["pass_percentage"] = ((device["passed_test"] / device["total_test"]) * 100 if device["total_test"] > 0 else 0)
            if 3 <= device["voltage"] <= 5.5:
                test_report["voltage_status"] = "PASS"
            else:
                test_report["voltage_status"] = "FAIL"

            if device["current"] < 10:
                test_report["current_status"] = "PASS"
            else:
                test_report["current_status"] = "FAIL"

            if 0 <= device["temperature"] <= 70:
                test_report["temperature_status"] = "PASS"
            else:
                test_report["temperature_status"] = "FAIL"
        
            if (test_report["voltage_status"] == "PASS" and test_report["current_status"] == "PASS" and test_report["temperature_status"] == "PASS" and test_report["power"] < 10 and test_report["pass_percentage"] >= 95):
                test_report["overall_status"] = "PASS"
            else:
                test_report["overall_status"] = "FAIL"
